...
Sprite* Sprite::createWithRenderTexture(RenderTexture* renderTexture, const Vec3& position, const Color4& color /* = Color4(1.0f,1.0f,1.0f,1.0f) */)
{
	auto sprite = new (std::nothrow) Sprite(position, renderTexture->getTexture(), color);
	if (not sprite)
	{
		return nullptr;
	}
...
Sprite* Sprite::create(const std::string& name, const Vec3& position, const Color4& color/* = Color4(1.0f,1.0f,1.0f,1.0f)*/)
{
	...
	auto sprite = new (std::nothrow) Sprite(position, texture, color);
	if (sprite)
	{
...
void Sprite::setGLProgram(const char* vShader, const char* fShader)
{
